...
object ScalaJS {
  ...
  object ScalaJSKeys {
    ...
    val scalaJSOutput = SettingKey[File]("scala-js-output")
  }
  ...
  val baseScalaJSSettings: Seq[Setting[_]] = Seq(
    ...
    scalaJSOutput <<= (sourceDirectory in Compile) {
      base => base.getParentFile / "public" / "javascripts"
    },

    scalaJSFiles <<= (sourceDirectory in Compile, streams, compileInputs in Compile, scalaJSOutput in Compile) map {
      (base, s, inputs, output) =>
        val files = (base.getParentFile / "assets" ** "*.scala").get
